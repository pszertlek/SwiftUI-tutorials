...
func load<T: Decodable>(_ filename: String, as type: T.Type = T.self) -> T {
    ...
    do {
        ...
        fatalError("Could't find \(filename) in main bundle:\(error)")
...
final class ImageStore {
    fileprivate typealias _ImagDictionary = [String: [Int: CGImage]]
    fileprivate var images: _ImagDictionary = [:]
    ...
    private func _guaraneeInitialImage(name: String) -> _ImagDictionary.Index {
        ...
        guard let url = Bundle.main.url(forResource: name, withExtension: "jpg"),
            ...
            let image = CGImageSourceCreateImageAtIndex(imageSource, 0, nil) else {
                fatalError("Could't load image \(name).jpg from main bundle")
        }
    ...
    func image(name: String, size: Int) -> Image {
        let index = _guaraneeInitialImage(name: name)
